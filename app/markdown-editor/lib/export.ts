...
// Both build the same self-contained HTML; PDF just routes it through
// the browser's print dialog (no library needed, perfect rendering).
...
export function exportHtml(options: ExportOptions): void {
  const fullHtml = buildStandaloneHtml(options);
  const blob = new Blob([fullHtml], { type: "text/html;charset=utf-8" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = `${options.title || "document"}.html`;
